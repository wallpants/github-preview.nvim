...
export function getScrollOffsets(
   ...
   for (let index = 0, len = sortedElements.length; index < len; index++) {
      ...
      const isFencedCodeInMarkdown =
         !isCodeFile && element.tagName === "PRE" && element.firstElementChild?.tagName === "CODE";
...
export function scroll(
   ...
   markdownContainerElement.scrollTo({
      top:
         cursorLineOffset[0] + markdownContainerElement.offsetTop - window.screen.height * percent,
