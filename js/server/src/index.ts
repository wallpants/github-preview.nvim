...
function devSafeParse(schema: ObjectSchema<any>, data: unknown) {
...
function main() {
    ipc.serve(function () {
        ...
        ipc.server.on(updateConfig, function (data: PluginProps, _socket: Socket) {
            devSafeParse(PluginPropsSchema, data);
            logger.verbose(updateConfig, data);
        });
        ...
        ipc.server.on(cursorMove, function (data: CursorMove, _socket: Socket) {
            devSafeParse(CursorMoveSchema, data);
            logger.verbose(cursorMove, data);
        });

        const contentChange: (typeof IPC_EVENTS)[number] = "github-preview-content-change";
        ipc.server.on(contentChange, function (data: ContentChange, _socket: Socket) {
            devSafeParse(ContentChangeSchema, data);
            logger.verbose(contentChange, data);
        });
    });
